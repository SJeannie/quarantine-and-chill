...
exports.JSONPatchOT = exports.jsonpatch = undefined;
...
const jsonpatch =
/******/
exports.jsonpatch = function (modules) {
  // webpackBootstrap
...
function (module, exports) {
  ...
  */
  ...
  function _deepClone(obj) {
    switch (typeof obj) {
      case "object":
        if (obj instanceof _core.DateTime) return obj;
        if (obj instanceof _core.Duration) return obj;
        if (!obj) return obj;

        if (obj['[[attributes]]']) {
          return obj; // let Class = obj.constructor
        ...
        return (0, _core.map)(obj, (k, value) => _deepClone(value));
  ...
  function deepMerge(obj1, obj2, cache = {
    ...
    switch (typeof obj2) {
      case "object":
        if (obj2 instanceof _core.DateTime) return resolve(obj2);
        if (obj2 instanceof _core.Duration) return resolve(obj2);
        ...
        if (Array.isArray(obj2)) {
          let ref = resolve(new _core.List());
          obj2.forEach((obj2, i) => ref.push(obj1[i] && obj1[i].uid == obj2.uid ? deepMerge(obj1[i], obj2, cache) : obj2));
          return ref;
        }

        let ref = resolve({});
        Object.assign(ref, obj1, (0, _core.map)(obj2, (k, obj2) => deepMerge(obj1[k], obj2, cache)));
        return ref;
  ...
  exports._deepClone = _deepClone; //3x faster than cached /^\d+$/.test(str)
  ...
  */
  ...
  var PatchError = function (_super) {
    ...
    function PatchError(message, name, index, operation, tree) {
      _super.call(this, patchErrorMessageFormatter(message, {
        name: name,
        index: index,
        operation: operation,
        tree: tree
...
const JSONPatchOT = exports.JSONPatchOT = function () {
  var debug = false;
  var JSONPatchOT = JSONPatchOT || {};

  JSONPatchOT.transform = function (sequenceA, sequences) {
    var concatAllSequences = [];
    concatAllSequences = concatAllSequences.concat.apply(concatAllSequences, sequences); // var clonedPatch = JSON.parse(JSON.stringify(this.patch)); // clone needed for debugging and visualization
    ...
    return result; // return new JSONPatchOperation(result, this.localRevision, operations[operations.length-1].localRevision, this.localRevPropName, this.remoteRevPropName);
  };
  ...
  var composeJSONPatches = function (original, operationObj) {
    ...
    if (transformAgainst[operationObj.op]) {
      // if we have any function to transform operationObj.op at all
      if (typeof transformAgainst[operationObj.op] == "function") {
        //not perfectly performant but gives easier maintenance and flexibility with transformations
        transformAgainst[operationObj.op](operationObj, original);
      } else {
        var orgOpsLen = original.length,
            currentOp = 0;
  ...
  var transformAgainst = {
    remove: function (patchOp, original) {
      ...
      var orgOpsLen = original.length,
          currentOp = 0,
          originalOp; // remove operation objects
      ...
      while (originalOp = original[currentOp]) {
        ...
        if ((originalOp.op === 'add' || originalOp.op === 'test') && patchOp.path === originalOp.path) {// do nothing ? (tomalec)
        } else // node in question was removed
          if (originalOp.from && (originalOp.from === patchOp.path || originalOp.from.indexOf(patchOp.path + "/") === 0) || patchOp.path === originalOp.path || originalOp.path.indexOf(patchOp.path + "/") === 0) {
      ...
      if (lastSlash > -1) {
        ...
        if (isValidIndex(index)) {
          ...
          while (currentOp < orgOpsLen) {
            ...
            if (originalOp.from && originalOp.from.indexOf(arrayPath) === 0) {
              //item from the same array
    ...
    replace: function (patchOp, original) {
      ...
      var currentOp = 0,
          originalOp; // remove operation objects withing replaced JSON node

      while (originalOp = original[currentOp]) {
        // TODO: `move`, and `copy` (`from`) may not be covered well (tomalec)
        debug && console.log("TODO: `move`, and `copy` (`from`) may not be covered well (tomalec)"); // node in question was removed
        // IT:
        // if( patchOp.path === originalOp.path || originalOp.path.indexOf(patchOp.path + "/") === 0 ){

        if (originalOp.from && (originalOp.from === patchOp.path || originalOp.from.indexOf(patchOp.path + "/") === 0) || originalOp.path.indexOf(patchOp.path + "/") === 0) {
          debug && console.log("Removing ", originalOp);
          callSplice(original, currentOp, 1);
          currentOp--;
        }
  ...
  function replacePathIfHigher(path, repl, index, modifier) {
    var result = path.substr(repl.length); // var match = result.match(/^(\d+)(.*)/);
