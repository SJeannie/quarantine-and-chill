...
const startApi = exports.startApi = () => {
  const program = {
    watch: './src',
    args: ['./src/Api.js']
  ...
  function handleChange(file) {
    ...
    delete errors[absoluteFile]; // file is in use by the app, let's restart!
  ...
  function generateTempFilename() {
    const now = new Date();
    return path.join(os.tmpdir(), [now.getYear(), now.getMonth(), now.getDate(), '-', process.pid, '-', (Math.random() * 0x100000000 + 1).toString(36)].join(''));
  }
  ...
  function restartAppInternal() {
    ...
    if (program.debug) {
      runnerExecArgv.push(typeof program.debug === 'boolean' ? `--debug` : `--debug=${program.debug}`);
    ...
    if (program.inspect) {
      ...
      const inspectArg = typeof program.inspect === 'boolean' ? `--inspect` : `--inspect=${program.inspect}`;
      runnerExecArgv.push(inspectArg);
    } // Support for --inspect-brk option


    if (program.inspectBrk) {
      const inspectBrkArg = typeof program.inspectBrk === 'boolean' ? `--inspect-brk` : `--inspect-brk=${program.inspectBrk}`;
      runnerExecArgv.push(inspectBrkArg);
    }

    const app = fork(path.resolve(__dirname, '__runner__.js'), {
